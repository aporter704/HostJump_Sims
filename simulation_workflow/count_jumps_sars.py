...
path_to_template_file = "/home/sebastiand/Dropbox/projects_WORKING/speciesJumpSimulations/HostJump_Sims/master_scripts/final_SARSCoV2_speciesjump_common.xml"
database_name = "sars.db"
dataset = "sars_common"
...
for i in range(1000):
    ...
    prune_high_prop = select_tips_to_remove(multitype_tree, "1", 0.5, opportunistic=False)
    ...
    prune_low_prop = select_tips_to_remove(multitype_tree, "1", 0.05, opportunistic=False)
    ...
    first_target_date = tip_ages.date[tip_ages.type == "1"].min()

    prune_opportunistic_high = select_tips_to_remove(multitype_tree, location_to_retain = "1", sampling_proportion = 0.5, opportunistic=True, sampling_start_age = first_target_date)
    ...
    prune_opportunistic_low = select_tips_to_remove(multitype_tree, location_to_retain = "1", sampling_proportion = 0.05, opportunistic=True, sampling_start_age = first_target_date)
    ...
    true_migration_0_1 = true_num_migration_events.loc[true_num_migration_events.ancestor_type == "0", ].shape[0]
    true_migration_1_0 = true_num_migration_events.loc[true_num_migration_events.ancestor_type == "1", ].shape[0]
    true_first_migration_age = true_num_migration_events.time[true_num_migration_events.ancestor_type == "0"].min()

    nontype_migration_0_1 = count_migrations_nonmultitype(nontyped_tree, "type1")
    nontype_num_migrations_0_1 = nontype_migration_0_1[0]
    nontype_first_migration_0_1 = nontype_migration_0_1[1]

    prune_high_prop_migration_0_1 = count_migrations_nonmultitype(tree_prune_high_prop, "type1")
    ...
    prune_low_prop_migration_0_1 = count_migrations_nonmultitype(tree_prune_low_prop, "type1")
    prune_low_num_migrations_0_1 = prune_low_prop_migration_0_1[0]
    prune_low_first_migration_0_1 = prune_low_prop_migration_0_1[1]

    prune_opportunistic_high_migration_0_1 = count_migrations_nonmultitype(tree_prune_opportunistic_high, "type1")
    ...
    prune_opportunistic_low_migration_0_1 = count_migrations_nonmultitype(tree_prune_opportunistic_low, "type1")
    ...
    cur = con.cursor() 
    table_name = dataset 
    cur.execute("CREATE TABLE IF NOT EXISTS "+table_name+"(tree_name, true_migration_from1_to2, true_migration_from2_to1, true_age_first_migration_from1_to2, nontyped_migration_from1_to2, nontyped_age_first_migration_from1_to_2,prune_high_prop_migration_from1_to2, prune_high_prop_first_migration_from1_to2,prune_low_prop_migration_from1_to2, prune_low_prop_first_migration_from1_to2,prune_opportunistic_high_migration_from1_to2, prune_opportunistic_high_first_migration_from1_to2, prune_opportunistic_low_migration_from1_to2, prune_opportunistic_low_first_migration_from1_to2);")
    con.commit()
    cur.execute("INSERT INTO "+table_name+" VALUES ("+"\""+dataset+str(i)+"\","+str(true_migration_0_1)+","+str(true_migration_1_0)+","+str(true_first_migration_age)+","+str(nontype_num_migrations_0_1)+","+str(nontype_first_migration_0_1)+","+str(prune_high_num_migrations_0_1)+","+str(prune_high_first_migration_0_1)+","+str(prune_low_num_migrations_0_1)+","+str(prune_low_first_migration_0_1)+","+str(prune_opportunistic_high_num_migrations_0_1)+","+str(prune_opportunistic_high_first_migration_0_1)+","+str(prune_opportunistic_low_num_migrations_0_1)+","+str(prune_opportunistic_low_first_migration_0_1)+")")
    con.commit()    
