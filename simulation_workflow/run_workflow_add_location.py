...
import os, sys, re, subprocess, sqlite3, argparse

parser = argparse.ArgumentParser()
parser.add_argument('-pre', '--pre', help = 'prefix used for output files and tables', required = True)

# The parameters below will set ups simulations:
template = 'final_HeV_speciesjump_common.xml' # template for running simulation, usually a MASTER file 
...
clock_rate = '6.5e-4' #1e-5 clock rate in subs/site/unit time
genome_size = '18000' #197417 genome size in nt
...
target_type = 'type2' # type label to match. In tree_post_processing.py we match the 'location' tag
location_type = '2' # We need this to match the subsampling script 
database_name = '/data/gpfs/projects/punim1405/species_jumps/HeV_common.db'
...
tree_processing = re.split(' |\n', os.popen('./tree_post_processing.py -t ' + output_name + '.nexus.tree -lh '+location_type+' -rc '+clock_rate+' -gl '+genome_size+' -seqGen ' + seqgen_path).read()) # manipulate trees

sim_tree_file = tree_processing[-4]
sim_aln_file = tree_processing[-2]
       
subsampling_command = './subsample_trees.py -t ' + sim_tree_file + ' -s ' + sim_aln_file + ' -lh '+ location_type + ' -ph ' + '0.8' + ' -pl ' + '0.05'     
